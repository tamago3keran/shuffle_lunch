class Admin::GroupSetsController < ApplicationController
  ...
  def create
    ...
    redirect_to admin_group_set_path(group_set)
  end

  def destroy
    if @group_set.groups.map(&:delete) && @group_set.destroy
      redirect_to admin_group_sets_path, flash: { notice: "チーム分けの削除に成功しました!" }
    else
      redirect_to admin_group_sets_path, flash: { error: "チーム分けの削除に失敗しました!" }
    end
  end
